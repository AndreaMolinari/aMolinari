...
const initResponse: Response<any> = {
  data: null,
  error: null
}
...
const useGitHub = () => {
  const [repos, setRepos] = React.useState<Response<any>>(initResponse);
  const [languages, setLanguages] = React.useState<Response<string[]>>(initResponse);
      ...
      graphql(
        ...
        .then((r) => {
          if (r.viewer.repositories.nodes) {
            setRepos(
              {
                data: r.viewer.repositories.nodes,
                error: null,
              }
              // .map((i) => i.name)
            );
